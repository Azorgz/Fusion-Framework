...
class C2RF(nn.Module):
    ...
    def forward(self, vi, ir):
        ...
        F_i1, F_i2, F_ib, F_id, F_v1, F_v2, F_vb, F_vd = self.AE_encoder(ir_Y, vi_Y)
        deformation = self.reg_net(F_ib, F_vb, type=self.direction)  # vi'-->ir
        disp = deformation[self.direction]
        if self.direction == 'vis2ir':
            vi, flow = self.ST(vi, disp)
            F_v2, flow = self.ST(F_v2, disp)
            F_vb, flow = self.ST(F_vb, disp)
            F_vd, flow = self.ST(F_vd, disp)
            reg = vi
        else:
            ir, flow = self.ST(ir, disp)
            F_i2, flow = self.ST(F_i2, disp)
            F_ib, flow = self.ST(F_ib, disp)
            F_id, flow = self.ST(F_id, disp)
            reg = ir
        ...
        vi_reg_Y, vi_reg_Cb, vi_reg_Cr = RGB2YCrCb(vi)
        fusion_reg_RGB = YCbCr2RGB(fusion_reg, vi_reg_Cb, vi_reg_Cr)

        return reg, fusion_reg_RGB, disp, flow
