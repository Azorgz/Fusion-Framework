...
import cv2
from code_pgmr.utils import randflow, randrot, randfilp
...
class RegData_MSRS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert os.path.basename(vi_path) == os.path.basename(
            ir_path), f"Mismatch ir:{os.path.basename(ir_path)} vi:{os.path.basename(vi_path)}."
...
class RegData_SAR(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert os.path.basename(vi_path) == os.path.basename(
            sar_path), f"Mismatch sar:{os.path.basename(sar_path)} vi:{os.path.basename(vi_path)}."
...
class RegData_PET_MRI(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert os.path.basename(PET_path) == os.path.basename(
            MRI_path), f"Mismatch MRI:{os.path.basename(MRI_path)} PET:{os.path.basename(PET_path)}."
...
class RegData_SPECT_MRI(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert os.path.basename(SPECT_path) == os.path.basename(
            MRI_path), f"Mismatch MRI:{os.path.basename(MRI_path)} SPECT:{os.path.basename(SPECT_path)}."
...
class TestData(torch.utils.data.Dataset):
    ...
    def __init__(self, opts, test_path):
        ...
        base_path = os.getcwd()
        self.target_folder = os.path.join(base_path, test_path, 'target')
        self.orign_folder = os.path.join(base_path, test_path, 'orign')
        self.crop = torchvision.transforms.RandomCrop(256)
    ...
    def __getitem__(self, index):
        # gain image path
        image_name_ori = self.orign_list[index]
        image_name_tgt = self.target_list[index]
        target_path = os.path.join(self.target_folder, image_name_tgt)
        orign_path = os.path.join(self.orign_folder, image_name_ori)
        # read image as type Tensor
        target = self.imread(path=target_path)
        ...
        return orign_warped, target, orign, image_name_tgt, disp
