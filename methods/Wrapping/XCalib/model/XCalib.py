...
from .spatial_transformer import depth_warp
...
class XCalib(nn.Module):
    ...
    def __init__(self, *args, cfg=None):
        ...
        self.scheduler = None
        self.optimizer = None
        self.path = os.getcwd() + "/methods/Wrapping/XCalib/" + cfg.output + '/' + cfg.data.name
        ...
        # Data
        self.number_cameras = len(self.cameras.cameras) if self.cameras.cameras is not None else 0
        cfg.train_collector.nb_cam = cfg.val_collector.nb_cam = self.number_cameras
...
class DataCollector:
    ...
    def add(self, batch: Batch):
        for i, (images, indices, cam, mod) in enumerate(zip(batch.images, batch.indices, batch.cameras, batch.modality)):
            ...
            if 'depths' in cam_buffer and batch.depths[i] is not None:
                cam_buffer['depths'] = cam_buffer['depths'] + (batch.depths[i], indices)
            if 'flows' in cam_buffer and batch.flows[i] is not None:
