...
class BackboneZoe(Backbone[BackboneZoeCfg]):
    ...
    def buildcore(self):
        sys.path.append(os.getcwd() + '/methods/Wrapping/XCalib/model/backbone/Depth_anything/metric_depth')
        parser = get_config('zoedepth', "infer")
        config = configure_parser(parser,
                                  None,
                                  path_config=os.getcwd() + '/methods/Wrapping/XCalib/model/backbone/Depth_anything/config_Depth_anything.yml',
                                  dict_vars=None)
