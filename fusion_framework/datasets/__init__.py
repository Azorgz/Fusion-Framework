...
def collate_ImageTensor(batch):
    ...
    """
    # items = batch[0]
    # if len(items) == 4:  # If the dataset returns 4 items (e.g., LYNRED_DETECTION)
    #     images_vis = [item[0] for item in batch]
    #     images_ir = [item[1] for item in batch]
    #     images_ir_16bits = [item[2] for item in batch]
...
def get_dataloaders(opt):
    ...
    for dataset in datasets:
        if dataset.lower() in DATASETS:
            ...
            dataloaders[dataset_instance.name] = DataLoader(dataset_instance,
                                              batch_size=1,
                                              shuffle=shuffle,
