...
class SuperFusion(nn.Module):
    def __init__(self, opt=None):
        ...
        # parameters
        lr = 0.001
        self.direction = opt.direction  # 'ir2vis' or 'vis2ir'
        # encoders
    ...
    def forward(self, ir, vi):
        ...
        if self.direction == 'ir2vis':
            ...
            vi = self.ST(vi, disp)
        return self.fusion_forward(ir, vi)

    def registration_forward(self, ir, vi, other=None):
        disp = self.DM(ir, vi, type=self.direction)[self.direction]
        if self.direction == 'ir2vis':
            reg = self.ST(ir, disp)
        else:
    ...
    def resume(self, model_dir, train=True):
        ...
        # optimizer
        if train:
            # self.DM_opt.load_state_dict(checkpoints['DM_opt'])
            # self.FN_opt.load_state_dict(checkpoints['FN_opt'])
