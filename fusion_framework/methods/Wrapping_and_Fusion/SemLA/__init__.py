...
def get_model(device, opt, task=None, **kwargs):
    ...
    class Model(nn.Module):
        ...
        def forward(self, img_vis, img_ir):
            ...
            Cb_numpy = np.uint8(Cb.squeeze().cpu().numpy() * 255)
            Cr_numpy = np.uint8(Cr.squeeze().cpu().numpy() * 255)
            ...
            if task != 'fusion':
                ...
                if self.direction == 'ir2vis':
                    ...
                    Cb = torch.from_numpy(tps.warpImage(Cb_numpy))[None][None].to(device)
                    Cr = torch.from_numpy(tps.warpImage(Cr_numpy))[None][None].to(device)
                    ...
                    img_vis = YCbCr2RGB(Y, Cb, Cr)
            if self.task == 'wrapping':
                return (img_vis, ImageTensor(img_ir).RGB('gray')) if self.direction == 'ir2vis' else (ImageTensor(img_vis), img_ir)

            fuse = self.model.fusion(torch.cat((Y, img_ir), dim=0), sa_ir, matchmode=self.match_mode).detach()
            fuse = YCbCr2RGB(fuse, Cb, Cr)
            return fuse, ImageTensor(img_vis), ImageTensor(img_ir).RGB('gray')
