...
from ..code_pgmr.modules import U_net, gaussian_weights_init, get_scheduler
import kornia.filters as KF
from ..code_pgmr.utils import RGB2YCrCb, YCbCr2RGB
from ..code_pgmr.loss import *
...
class Conv2d(nn.Module):
    def __init__(self, n_in, n_out, kernel_size, stride=1, padding=0, dilation=1, norm=None, act=nn.LeakyReLU,
                 bias=False):
...
class regis_model(nn.Module):
    def __init__(self, model_orign, model_target, unshare_depth=4, matcher_depth=4, num_pyramids=2, device='cpu'):
        ...
        #vis-ir,1_ir,2_vis
        self.feat31_PGB = PromptGenBlock(prompt_dim=base_oc * 8, prompt_len=5, prompt_size=96, lin_dim=base_oc * 8).to(
            device)
        self.feat32_PGB = PromptGenBlock(prompt_dim=base_oc * 8, prompt_len=5, prompt_size=96, lin_dim=base_oc * 8).to(
            device)
        self.feat21_PGB = PromptGenBlock(prompt_dim=base_oc * 4, prompt_len=5, prompt_size=96, lin_dim=base_oc * 4).to(
            device)
        self.feat22_PGB = PromptGenBlock(prompt_dim=base_oc * 4, prompt_len=5, prompt_size=96, lin_dim=base_oc * 4).to(
            device)
        self.feat11_PGB = PromptGenBlock(prompt_dim=base_oc * 2, prompt_len=5, prompt_size=96, lin_dim=base_oc * 2).to(
            device)
        self.feat12_PGB = PromptGenBlock(prompt_dim=base_oc * 2, prompt_len=5, prompt_size=96, lin_dim=base_oc * 2).to(
            device)
        #SPECT-MRI,1_MRI,2_SPECT
        self.feat31_PGB_SPECT_MRI = PromptGenBlock(prompt_dim=base_oc * 8, prompt_len=5, prompt_size=96,
                                                   lin_dim=base_oc * 8).to(device)
        self.feat32_PGB_SPECT_MRI = PromptGenBlock(prompt_dim=base_oc * 8, prompt_len=5, prompt_size=96,
                                                   lin_dim=base_oc * 8).to(device)
        self.feat21_PGB_SPECT_MRI = PromptGenBlock(prompt_dim=base_oc * 4, prompt_len=5, prompt_size=96,
                                                   lin_dim=base_oc * 4).to(device)
        self.feat22_PGB_SPECT_MRI = PromptGenBlock(prompt_dim=base_oc * 4, prompt_len=5, prompt_size=96,
                                                   lin_dim=base_oc * 4).to(device)
        self.feat11_PGB_SPECT_MRI = PromptGenBlock(prompt_dim=base_oc * 2, prompt_len=5, prompt_size=96,
                                                   lin_dim=base_oc * 2).to(device)
        self.feat12_PGB_SPECT_MRI = PromptGenBlock(prompt_dim=base_oc * 2, prompt_len=5, prompt_size=96,
                                                   lin_dim=base_oc * 2).to(device)
        #CT-MRI,2_PET
        self.feat32_PGB_PET = PromptGenBlock(prompt_dim=base_oc * 8, prompt_len=5, prompt_size=96,
                                             lin_dim=base_oc * 8).to(device)
        self.feat22_PGB_PET = PromptGenBlock(prompt_dim=base_oc * 4, prompt_len=5, prompt_size=96,
                                             lin_dim=base_oc * 4).to(device)
        self.feat12_PGB_PET = PromptGenBlock(prompt_dim=base_oc * 2, prompt_len=5, prompt_size=96,
                                             lin_dim=base_oc * 2).to(device)
        #VI-SAR,1_SAR
        self.feat31_PGB_SAR = PromptGenBlock(prompt_dim=base_oc * 8, prompt_len=5, prompt_size=96,
                                             lin_dim=base_oc * 8).to(device)
        self.feat21_PGB_SAR = PromptGenBlock(prompt_dim=base_oc * 4, prompt_len=5, prompt_size=96,
                                             lin_dim=base_oc * 4).to(device)
        self.feat11_PGB_SAR = PromptGenBlock(prompt_dim=base_oc * 2, prompt_len=5, prompt_size=96,
                                             lin_dim=base_oc * 2).to(device)
    ...
    def match(self, feat11, feat12, feat21, feat22, feat31, feat32):
        ...
        feat21 = F.grid_sample(feat21, self.grid_down + disp2.permute(0, 2, 3, 1), align_corners=True)
        ...
        #warp
        feat11 = F.grid_sample(feat11, self.grid_full + (disp1 + disp2).permute(0, 2, 3, 1), align_corners=True)
...
class PGMR(nn.Module):
    def __init__(self, opts=None, device=None, direction='ir2vis'):
        ...
        self.DM = regis_model(model_orign=self.model_orign, model_target=self.model_target, device=device)  #配准
    ...
    def generate_mask(self):
        ...
        flow = self.ST.grid - self.disp
        goodmask = F.grid_sample(self.goodmask, flow, align_corners=True)
        self.goodmask_inverse = goodmask

    def forward(self, ir, vi):
        disp = self.DM(ir, vi)[self.direction]
    ...
    def registration_forward(self, ir, vi):
        disp = self.DM(ir, vi, type=self.direction)[self.direction]
        if self.direction == 'ir2vis':
            reg = self.ST(ir, disp)
        else:
            reg = self.ST(vi, disp)
