...
from fusion_framework.methods.Fusion.MaeFuse import fusion
from fusion_framework.methods.Fusion.MaeFuse.connect_net import ConnectModel_MAE_Fuion as ConnectModel_MAE_Fusion
from fusion_framework.methods.Fusion.MaeFuse.test_fusion import prepare_model
...
def get_model(device, opt, **kwargs):
    ...
    checkpoint = torch.load(str(ROOT_DIR) + '/checkpoints/final_new_60.pth',
    ...
    class Model(torch.nn.Module):
        ...
        def forward(self, img_vis, img_ir):
            ...
            fused_img = ImageTensor(torch.cat((pred, vi_Cb, vi_Cr), dim=1), colorspace='YCbCr').RGB().resize(size)
