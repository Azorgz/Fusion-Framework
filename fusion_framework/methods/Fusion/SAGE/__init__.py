...
from fusion_framework.methods.Fusion.SAGE.model_sub.model import Network


def get_model(device, opt, **kwargs):
    model = Network()
    model.load_state_dict(torch.load(os.getcwd() + '/fusion_framework/methods/Fusion/SAGE/checkpoints/checkpoints.pt', weights_only=True))
    model.eval()
