...
def get_model(device, opt, **kwargs):
    ...
    lut = load_lookup_table(str(ROOT_DIR) + '/ckpts/fine_tuned_lut_original.npy').to(device)
    if lut is None:
        return

    get_context = Generator_for_info()
    get_context.load_state_dict(torch.load(str(ROOT_DIR) + '/ckpts/generator_context_original.pth', map_location='cpu'))
    ...
    class Model(torch.nn.Module):
        ...
        def forward(self, img_vis, img_ir):
            ...
            return fused_result.clamp(0, 1), img_vis, img_ir
